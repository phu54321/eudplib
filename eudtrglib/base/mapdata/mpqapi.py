'''
SFmpq.dll wrapper. Used internally inside eudtrglib.
'''
...
def InitMpqLibrary():
    ...
    try:
        ...
        SFmpq.SFileReadFile.argtypes     = [c_int, c_char_p, c_int, c_void_p, c_int]
        ...
        SFmpq.MpqAddFileFromBuffer.argtypes    = [c_int, c_void_p, c_int, c_char_p, c_int, c_int, c_int]
        SFmpq.MpqAddWaveFromBuffer.argtypes    = [c_int, c_void_p, c_int, c_char_p, c_int, c_int]
...
class MpqRead:
    ...
    def Extract(self ,fname):
        ...
        # Open file
        fileh = c_int()
        ret = self.SFmpq.SFileOpenFileEx(self.mpqh, u2b(fname), 0, byref(fileh))
        if not ret:
            return None
        fileh = fileh.value
        ...
        while readleft > 0:
            ...
            readbyte = c_int()
            ret = self.SFmpq.SFileReadFile(fileh, buffer, readleft, byref(readbyte), 0)
...
class MpqWrite:
    ...
    def PutFile(self, fname, buffer):
        ...
        flag = MAFA_MODCRYPTKEY | MAFA_ENCRYPT | MAFA_COMPRESS | MAFA_REPLACE_EXISTING
        cmptype = MAFA_COMPRESS_STANDARD

        ret = SFmpq.MpqAddFileFromBufferEx(self.mpqh, buffer, len(buffer), u2b(fname), flag, cmptype, 0)
        if not ret: return False
    ...
    def PutWave(self ,fname, buffer, complev = WAVCOMP_LOWQUALITY):
        ...
        ret = SFmpq.MpqAddWaveFromBuffer(self.mpqh, buffer, len(buffer), u2b(fname), flag, cmplevel)
        if not ret: return False
        return True
