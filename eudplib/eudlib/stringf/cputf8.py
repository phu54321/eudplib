...
def f_cp949_to_utf8_cpy(dst, src):
    ...
    if EUDInfLoop()():
        ...
        if EUDElse()():
            b2 = br1.readbyte()
            bw1.writebyte(convert_char_ksc5601_to_ucs2(b, b2))
        EUDEndIf()
    EUDEndInfLoop()
    bw1.writebyte(0)
    bw1.flushdword()
...
def convert_char_ksc5601_to_ucs2(b1, b2):
    code_ucs2 = c.EUDVariable()
    tab_idx = (b1 - 0x00a1) * 94 + (b2 - 0x00a1)
    if EUDIf()([tab_idx >= 0, tab_idx < ksc5601max]):
        code_ucs2 << tabksc5601[tab_idx]
        Trigger(
            code_ucs2.Exactly(-1),
            code_ucs2.SetNumber(0)
        )
    if EUDElse()():
        code_ucs2 << 0
    EUDEndIf()
